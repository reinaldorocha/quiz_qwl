...
import {
  type BuilderFeedbackVariant,
  useBuilderStore,
} from "@/domains/quiz/store/builder.store";
import { cn } from "@/lib/utils";

const VARIANT_STYLES: Record<
  BuilderFeedbackVariant,
  {
    label: string;
    accent: string;
    progress: string;
...
const DISMISS_MS: Record<BuilderFeedbackVariant, number> = {
  success: 4000,
  error: 6500,
  info: 4000,
};
...
export function BuilderToast() {
  const feedback = useBuilderStore((s) => s.feedback);
  const feedbackVariant = useBuilderStore((s) => s.feedbackVariant);
  ...
  function showToast(message: string, variant: BuilderFeedbackVariant) {
    isClosingRef.current = false;
    clearAllTimers();
    ...
    setIsExiting(false);
    setIsPaused(false);
    armAutoDismiss(DISMISS_MS[variant]);
  }

  useEffect(() => {
    if (!feedback) return;

    showToast(feedback, feedbackVariant);
    setFeedback(null);
  }, [feedback, feedbackVariant, setFeedback]);
  ...
  return (
    <div
      className={cn(
        "pointer-events-auto fixed bottom-5 left-5 z-50 w-[min(100vw-2.5rem,22rem)] overflow-hidden rounded-2xl border border-white/8 bg-slate-950/92 shadow-[0_18px_50px_-12px_rgba(0,0,0,0.65)] ring-1 backdrop-blur-md",
        variant.ring,
    ...
    >
      ...
      <div className="relative flex items-start gap-3 px-4 pt-3.5 pb-3">
        ...
        >
          <Icon className={cn("size-4", variant.iconClass)} />
        </div>
